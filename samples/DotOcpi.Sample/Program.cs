...
app.MapGet("/api/events", async (SseEventService sse, HttpContext ctx) =>
{
...
app.MapGet("/api/metrics", (ICpoRegistry registry, InMemoryDataStore store, MetricsCollector metrics) =>
    Results.Ok(new
    {
...
app.MapPost("/api/cpos/register", async (
    HttpContext ctx,
    ...
    SampleTokenProvider tokenProvider) =>
{
    ...
    if (string.IsNullOrEmpty(versionsUrl) || string.IsNullOrEmpty(tokenA))
        return Results.Content("<span class='badge badge-error'>versionsUrl and tokenA are required.</span>", "text/html");
    ...
    try
    {
        ...
        if (negotiated is null) return Results.BadRequest("No mutually supported OCPI version.");
        ...
        var cpoResponse = await credentialsClient.PostCredentialsAsync(
            credentialsUrl, tokenA, negotiated.Value, credentials);
        ...
        return Results.Content($"<span class='badge badge-success'>Registered {connection.ConnectionKey} (OCPI {negotiated.Value.ToVersionString()})</span>", "text/html");
    }
    catch (Exception ex)
    {
...
app.MapPost("/api/commands/start-session", async (HttpContext ctx, IOcpiClient ocpiClient, ICpoRegistry registry, IOcpiSyncService syncService, CommandHistory cmdHistory) =>
{
    var f = await ctx.Request.ReadFormAsync();
    ...
    if (string.IsNullOrEmpty(cpoId)) cpoId = "DE:CPO";
    if (string.IsNullOrEmpty(locationId)) locationId = "LOC1";
    if (string.IsNullOrEmpty(evseUid)) evseUid = "DE*CPO*E001";
    if (string.IsNullOrEmpty(tokenUid)) tokenUid = "TOKEN001";
    ...
    try
    {
        ...
        object command = version.UsesPartyIdInUrls()
            ? new StartSession
            {
                ...
                Token = new Token
                {
                    CountryCode = new CiString("NL"), PartyId = new CiString("MSP"),
                    Uid = new CiString(tokenUid), Type = TokenType.RFID,
                    ContractId = new CiString("NL-MSP-000001"),
                    Issuer = "DotOcpi Sample eMSP", Valid = true,
                    Whitelist = WhitelistType.ALWAYS, LastUpdated = DateTimeOffset.UtcNow,
                },
            ...
            : new DotOcpi.Models.V2_1_1.StartSession
            {
                ...
                {
                    Uid = tokenUid, Type = DotOcpi.Models.V2_1_1.TokenType.RFID,
        ...
        return Results.Content(result.IsSuccess
    ...
    catch (Exception ex)
    {
        ...
        return Results.Content($"<div class='result-box'><span class='badge badge-error'>ERROR</span> {ex.Message}</div>", "text/html");
    }
});

app.MapPost("/api/commands/stop-session", async (HttpContext ctx, IOcpiClient ocpiClient, ICpoRegistry registry, IOcpiSyncService syncService, CommandHistory cmdHistory) =>
{
    var f = await ctx.Request.ReadFormAsync();
    var cpoId = f["cpoId"].ToString();
    ...
    if (string.IsNullOrEmpty(cpoId)) cpoId = "DE:CPO";
    if (string.IsNullOrEmpty(sessionId)) sessionId = "SES001";
...
app.MapGet("/api/evse-session", (HttpContext ctx, ICpoRegistry registry, InMemoryDataStore store) =>
{
    ...
    var matchingSessions = store.Sessions
        .Where(kv => kv.Key.StartsWith(cpoId + ":"))
        .Select(kv =>
        {
    ...
    foreach (var ses in matchingSessions)
    {
        ...
        var kwh = ses.TryGetProperty("kwh", out var kwhP) ? kwhP.GetDecimal().ToString("F1", System.Globalization.CultureInfo.InvariantCulture) : "0.0";
        var cost = "—";
        if (ses.TryGetProperty("total_cost", out var costP))
        {
            if (costP.ValueKind == JsonValueKind.Number)
                cost = costP.GetDecimal().ToString("F2", System.Globalization.CultureInfo.InvariantCulture) + " EUR";
        ...
        sb.Append("<div class='live-stat'><div class='live-stat-value'>").Append(kwh).Append("</div><div class='live-stat-label'>kWh</div></div>");
        sb.Append("<div class='live-stat'><div class='live-stat-value'>").Append(cost).Append("</div><div class='live-stat-label'>Cost</div></div>");
        sb.Append("<div class='live-stat'><div class='live-stat-value text-sm'>").Append(start).Append("</div><div class='live-stat-label'>Started</div></div>");
        sb.Append("</div>");

        sb.Append("<form hx-post='/api/commands/stop-session' hx-target='closest .evse-live-session' hx-swap='innerHTML' style='margin-top:0.5rem'>");
...
app.MapGet("/api/sessions-table", (HttpContext ctx, ICpoRegistry registry, InMemoryDataStore store) =>
{
    ...
    var sessions = store.Sessions
        ...
        .Where(x => string.IsNullOrEmpty(statusFilter) ||
            (x.Data.TryGetProperty("status", out var s) && string.Equals(s.GetString(), statusFilter, StringComparison.OrdinalIgnoreCase)))
        .ToList();

    var sb = new System.Text.StringBuilder();
    sb.Append("<div class='table-wrap'><table><thead><tr>");
    sb.Append("<th>ID</th><th>CPO</th><th>Location</th><th>EVSE</th><th>Status</th><th>kWh</th><th>Start</th><th>Cost</th><th>Actions</th>");
    sb.Append("</tr></thead><tbody>");

    if (sessions.Count == 0)
    {
        sb.Append("<tr><td colspan='9'><div class='empty-state'><div class='empty-state-icon'>▶</div><div class='empty-state-text'>No sessions tracked yet.</div></div></td></tr>");
    }
    else
    {
        foreach (var (cpoKey, ses) in sessions)
        {
            ...
            var status = ses.TryGetProperty("status", out var stP) ? stP.GetString() : "—";
            var kwh = ses.TryGetProperty("kwh", out var kwhP) ? kwhP.GetDecimal().ToString("F1", System.Globalization.CultureInfo.InvariantCulture) : "—";
            var start = ses.TryGetProperty("start_date_time", out var startP) ? startP.GetString() : "—";
            var cost = "—";
            if (ses.TryGetProperty("total_cost", out var costP))
            {
                if (costP.ValueKind == JsonValueKind.Number) cost = costP.GetDecimal().ToString("F2", System.Globalization.CultureInfo.InvariantCulture) + " EUR";
                else if (costP.ValueKind == JsonValueKind.Object && costP.TryGetProperty("excl_vat", out var excl)) cost = excl.GetDecimal().ToString("F2", System.Globalization.CultureInfo.InvariantCulture) + " EUR";
            }
            var badge = status switch { "ACTIVE" => "badge-active", "COMPLETED" => "badge-completed", "PENDING" => "badge-warning", _ => "badge-info" };
            ...
            sb.Append("<td><span class='badge ").Append(badge).Append("'><span class='badge-dot'></span> ").Append(status).Append("</span></td>");
            ...
            if (status == "ACTIVE")
            {
                sb.Append("<td><form hx-post='/api/commands/stop-session' hx-target='#stop-r-").Append(id).Append("' hx-swap='innerHTML' style='display:inline'>");
...
app.MapGet("/api/commands/history", (CommandHistory cmdHistory) =>
{
    var records = cmdHistory.GetRecent();
    var html = string.Join("", records.Select(r =>
    {
        var badgeClass = r.Result == "ACCEPTED" ? "badge-success" : r.Result == "ERROR" ? "badge-error" : "badge-warning";
    ...
    if (string.IsNullOrEmpty(html))
        html = "<tr><td colspan='4' class='text-center text-muted' style='padding: 1.5rem;'>No commands sent yet</td></tr>";
...
app.MapPost("/api/sync/{cpoId}/{module}", async (string cpoId, string module, IOcpiSyncService syncService) =>
{
...
app.MapPost("/api/sync/all", async (ICpoRegistry registry, IOcpiSyncService syncService) =>
{
...
static string ExtractLocationId(JsonElement ses)
{
    ...
    if (ses.TryGetProperty("location", out var loc) && loc.ValueKind == JsonValueKind.Object && loc.TryGetProperty("id", out var embeddedId))
...
static string ExtractEvseUid(JsonElement ses)
{
    ...
    if (ses.TryGetProperty("location", out var loc)
