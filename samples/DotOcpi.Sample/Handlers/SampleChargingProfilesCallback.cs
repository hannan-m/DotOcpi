...
/// </summary>
public sealed partial class SampleChargingProfilesCallback(ILogger<SampleChargingProfilesCallback> logger)
    : IChargingProfilesCallback
{
    ...
    [LoggerMessage(
        ...
        Message = "[ChargingProfiles] Result for {CorrelationId} from {CpoId}"
    )]
    private partial void LogProfileResult(string correlationId, string cpoId);

    [LoggerMessage(
        Level = LogLevel.Information,
        Message = "[ChargingProfiles] Active profile update for session {SessionId} from {CpoId}"
    )]
