...
/// </summary>
public sealed partial class SampleCdrsReceiver(InMemoryDataStore store, ILogger<SampleCdrsReceiver> logger)
    : ICdrsReceiver
{
    private readonly ILogger _logger = logger;

    public Task<OcpiResult<CdrPostResult>> OnCdrPostAsync(OcpiRequestContext context, object data, CancellationToken ct)
    {
    ...
    public Task<OcpiResult<object>> GetCdrAsync(OcpiRequestContext context, string cdrId, CancellationToken ct)
    {
        var key = InMemoryDataStore.Key(context.CpoId, cdrId);
