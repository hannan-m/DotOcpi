...
/// </summary>
public sealed partial class SampleLocationsReceiver(InMemoryDataStore store, ILogger<SampleLocationsReceiver> logger)
    : ILocationsReceiver
{
    ...
    [LoggerMessage(
        ...
        Message = "[Locations] PUT EVSE {EvseUid} at {LocationId} from {CpoId}"
    )]
    private partial void LogEvsePut(string evseUid, string locationId, string cpoId);

    [LoggerMessage(
        Level = LogLevel.Information,
        Message = "[Locations] PATCH EVSE {EvseUid} at {LocationId} from {CpoId}"
    )]
    private partial void LogEvsePatch(string evseUid, string locationId, string cpoId);

    [LoggerMessage(
        Level = LogLevel.Information,
        Message = "[Locations] PUT connector {ConnectorId} on EVSE {EvseUid} at {LocationId}"
    )]
    private partial void LogConnectorPut(string connectorId, string evseUid, string locationId);

    [LoggerMessage(
        Level = LogLevel.Information,
        Message = "[Locations] PATCH connector {ConnectorId} on EVSE {EvseUid} at {LocationId}"
    )]
